<x-admin-layout title="Add Tax Rate">
    <div class="py-8 px-4 sm:px-6 lg:px-8 space-y-6 max-w-4xl mx-auto">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-gray-900">Add New Tax Rate</h1>
        ...
        <div class="bg-white shadow sm:rounded-lg">
            <form action="{{ route('admin.tax-rates.store') }}" method="POST">
                @csrf
                {{-- Include the partial, passing the $taxRate from controller --}}
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 flex justify-end space-x-3">
                    ...
                    <x-primary-button>{{ __('Create Tax Rate') }}</x-primary-button>
                </div>
            </form>
        </div>
    </div>
